package Tencent.数组和字符串;

/**
 * Created by Ge YangChen
 * 2019/9/29 9:50
 */
/*请你来实现一个 atoi 函数，使其能将字符串转换成整数。

首先，该函数会根据需要丢弃无用的开头空格字符，直到寻找到第一个非空格的字符为止。

当我们寻找到的第一个非空字符为正或者负号时，则将该符号与之后面尽可能多的连续数字组合起来，作为该整数的正负号；假如第一个非空字符是数字，则直接将其与之后连续的数字字符组合起来，形成整数。

该字符串除了有效的整数部分之后也可能会存在多余的字符，这些字符可以被忽略，它们对于函数不应该造成影响。

注意：假如该字符串中的第一个非空格字符不是一个有效整数字符、字符串为空或字符串仅包含空白字符时，则你的函数不需要进行转换。

在任何情况下，若函数不能进行有效的转换时，请返回 0。

说明：

假设我们的环境只能存储 32 位大小的有符号整数，那么其数值范围为 [−2^31,  2^31 − 1]。如果数值超过这个范围，请返回  INT_MAX (231 − 1) 或 INT_MIN (−231) 。
*/
public class Q4_myAtoi {
    final int INT_MAX = 2147483647;
    final int INT_MIN = -2147483648;
    public int myAtoi(String str) {

        if(str==null){
            return 0;
        }
        String newStr = str.trim();
        int res = 0;

        if (newStr.isEmpty()) {
            return 0;
        } else if (newStr.charAt(0) == '-') {
            if (newStr.substring(1).length() == 0) {
                return 0;
            }
            try {
                res = 0 - str2Int(newStr.substring(1));
            } catch (NumberFormatException e) {
                return INT_MIN;
            }

        } else if (newStr.charAt(0) == '+') {
            if (newStr.substring(1).length() == 0) {
                return 0;
            }
            try {
                res = str2Int(newStr.substring(1));
            } catch (NumberFormatException e) {
                return INT_MAX;
            }
        } else if (Character.isDigit(newStr.charAt(0))) {
            try {
                res = str2Int(newStr);
            } catch (NumberFormatException e) {
                return INT_MAX;
            }
        } else return 0;
        return res;
    }

    public int str2Int(String s) {
        int len = s.length();
        int i=0;
        if (!Character.isDigit(s.charAt(0))) {
            return 0;
        }
        for (; i < len; i++) {
            if (!Character.isDigit(s.charAt(i))) {
                break;
            }
        }
        return Integer.parseInt(s.substring(0,i));
    }

    public static void main(String[] args) {
        Q4_myAtoi a = new Q4_myAtoi();
        String str = "-9128347212";
        String str2 = "+-2";
        String str3 = "+124";
        int num = a.INT_MIN -1;
        System.out.println(a.myAtoi(str));
        System.out.println(a.myAtoi(str2));
        System.out.println(a.myAtoi(str3));

    }
}
